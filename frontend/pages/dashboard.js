...
export default function Dashboard() {
  ...
  const fetchLeaves = async (token) => {
    try {
      const res = await axios.get("https://leave-management-backend-nmqs.onrender.com/api/leaves", {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) {
        await axios.put(`https://leave-management-backend-nmqs.onrender.com/api/leaves/${editId}`, form, { headers: getHeaders() });
        addToast("success", "Leave Updated!", "Your request has been updated.");
        setEditId(null);
      } else {
        await axios.post("https://leave-management-backend-nmqs.onrender.com/api/leaves", form, { headers: getHeaders() });
